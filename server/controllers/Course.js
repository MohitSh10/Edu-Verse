...
exports.createCourse = async (req, res) => {
  try {
    ...
    // Prefer uploaded file over URL string
    let thumbnailUrl = thumbnail || "";
    if (req.file) {
      const serverUrl = process.env.SERVER_URL || process.env.BACKEND_URL || "http://localhost:4000";
      thumbnailUrl = `${serverUrl}/uploads/${req.file.filename}`;
    }
...
exports.editCourse = async (req, res) => {
  try {
    ...
    if (req.file) {
      const serverUrl = process.env.SERVER_URL || process.env.BACKEND_URL || "http://localhost:4000";
      updates.thumbnail = `${serverUrl}/uploads/${req.file.filename}`;
    }
...
exports.createSubSection = async (req, res) => {
  try {
    ...
    const serverUrl = process.env.SERVER_URL || process.env.BACKEND_URL || "http://localhost:4000";
    const resolvedVideoUrl = req.file
...
exports.updateSubSection = async (req, res) => {
  try {
    ...
    if (req.file) {
      const serverUrl = process.env.SERVER_URL || process.env.BACKEND_URL || "http://localhost:4000";
      updates.videoUrl = `${serverUrl}/uploads/${req.file.filename}`;
    } else if (videoUrl) {
