...
export default function CourseForm() {
  ...
  const onSubmit = async (data) => {
    ...
    // text fields
    ["courseName", "description", "price", "level", "language", "whatYouWillLearn", "thumbnail"].forEach(
    ...
    // file upload overrides URL if provided
    if (data.thumbnailFile?.[0]) fd.append("thumbnail", data.thumbnailFile[0]);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-richblack-800 border border-richblack-700 rounded-xl p-6 space-y-5">
      ...
      {/* Thumbnail */}
      <div>
        <label className="block text-sm text-richblack-300 mb-1.5">Course Thumbnail</label>
        <input
          type="file"
          accept="image/*"
        ...
        />
        <p className="text-richblack-500 text-xs mt-1">— or paste a URL —</p>
        <input {...register("thumbnail")} className={inputCls} placeholder="https://example.com/image.jpg" />
        {course?.thumbnail && (
