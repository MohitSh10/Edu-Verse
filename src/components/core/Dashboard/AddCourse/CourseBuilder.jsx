...
function SubSectionModal({ sectionId, onClose, onSave, existing }) {
  ...
  const buildFormData = () => {
    ...
    if (videoFile) {
      fd.append("video", videoFile); // field name "video" → server reads req.files.video[0]
    } else if (videoUrl) {
      fd.append("videoUrl", videoUrl);
    }
    // PDF attachment (optional)
    if (pdfFile) {
      fd.append("pdf", pdfFile); // field name "pdf" → server reads req.files.pdf[0]
    }
    return fd;
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-richblack-800 border border-richblack-700 rounded-xl p-6 w-full max-w-md mx-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-richblack-300 mb-1.5">
              PDF Attachment <span className="text-richblack-500">(optional — lecture notes, slides, etc.)</span>
            </label>
            <input
              type="file"
              ...
              className="form-input cursor-pointer"
            />
            {existing?.resources?.filter((r) => r.type === "pdf").map((r, i) => (
              <p key={i} className="text-xs text-richblack-400 mt-1">
                Current PDF: <a href={r.url} target="_blank" rel="noreferrer" className="text-yellow-50 underline">{r.title}</a>
              </p>
            ))}
          </div>
